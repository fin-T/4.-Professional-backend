...
export class CommonService {
    ...
    private readonly EntitiesRepositoriesMap: {
        [key: string]: Repository<OneOfItems> |
        Repository<OneOfItemImages>
    } = {
    ...
    async getEntitiesByUrls(instanceEntity: OneOfAllTypes, urls: string[]) {
        try {
            let constructorName = instanceEntity.constructor.name;
            let repository = this.EntitiesRepositoriesMap[constructorName];
            let entities = [];
     ...
     */
    async getNonExistingItemUrls(data: OneOfCreateItemsDto | OneOfUpdateItemsDto): Promise<string[]> {
        try {
            let unexistUrls: string[] = [];
            for (let key in data) {
                if (key === 'homeworld' && typeof data[key] === 'string') {
                    unexistUrls.push(...await this.getNonExistingUrls(new Planets, [data[key]]));
                }
                let elem = data[key];
                if (Array.isArray(elem)) {
                    if (elem.length > 0) switch (key) {
                        case 'characters' || 'residents': {
                            unexistUrls.push(...await this.getNonExistingUrls(new People, elem)); break;
                        }
                        case 'films': unexistUrls.push(...await this.getNonExistingUrls(new Films, elem)); break;
                        case 'planets' || 'homeworld': {
                            unexistUrls.push(...await this.getNonExistingUrls(new Planets, elem)); break;
                        }
                        case 'species': unexistUrls.push(...await this.getNonExistingUrls(new Species, elem)); break;
                        case 'vehicles': unexistUrls.push(...await this.getNonExistingUrls(new Vehicles, elem)); break;
                        case 'starships': unexistUrls.push(...await this.getNonExistingUrls(new Starships, elem)); break;
                        default: break;
                    }
                }
            }
     ...
     * @returns Urls that do not exist in the database.
     */
    async getNonExistingUrls(instanceEntity: OneOfItems, searchUrls: string[]): Promise<string[]> {
        try {
            let constructorName = instanceEntity.constructor.name;
            let repository = this.EntitiesRepositoriesMap[constructorName];

            let urls = (await repository.find()).map((item: OneOfAllTypes) => item.url);
